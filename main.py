...
if "hof.txt" not in os.listdir("./"):
    open("hof.txt", "w", encoding="UTF-8").close()
...
class CustomBot(commands.Bot):
    def __init__(self, *args, **kwargs):
        ...
        # self.db = aiosqlite.connect("hochanic.db")
        self.screenshot_channels = get_bot_settings()["screenshot_channels"]

    def reload_channels(self):
        self.screenshot_channels = get_bot_settings()["screenshot_channels"]

    def get_hof_channel(self):
        return self.get_channel(get_bot_settings()["hof_channel"])

    def get_upvote_channel(self):
        return self.get_channel(get_bot_settings()["upvote_channel"])


bot = CustomBot(command_prefix=get_prefix, help_command=None)
